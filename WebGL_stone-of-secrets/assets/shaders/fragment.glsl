...
void main() {
 ...
 float ns = 0.3*pow(xfuel,3.0)*pow(ypart,2.0)*(smokeNoise+0.4*(1.0-noise));
 vec3 smoke = vec3(ns);
 //
 // sparks
 ...
 if (sparkLife>0.0) {
  ...
  sparksGray = max(0.0, 1.0 - sparkLength/(sparkSize*sparkGridSize));
  sparks = sparkLife*sparksGray*vec3(1.0,0.3,0.0);
 }
 //
 float transparency = max(max(f, sparksGray * sparkLife), ns);
 gl_FragColor = vec4(max(fire,sparks)+smoke, transparency);
}
